...
with open('hw5_6.txt', encoding='utf-8') as f:
    readed_list = f.readlines()
    subj_dict = {}
    for el in readed_list:
        subj, *quantity = el.split()
        subj_dict[subj] = quantity
    for key, value in subj_dict.items():
        el_sum = sum([int(el[:el.find('(')]) for el in value if '(' in el])
        subj_dict[key] = el_sum
print(subj_dict)
